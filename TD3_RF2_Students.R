#### 2eme partie
#### Comme dans la partie 1, les commentaires sont en anglais

data <- read.csv(row.names = 1,
                 text = "Ticker,Sector,MarketCap,Beta,GrossMargin,PEG,PE,PBR,DivYield,ROE,ROIC,Vol,FwdReturn,Return1m,Return12m,NetDE
...
lm_fit1 <- #Question 1: fit a linear model, FwdReturn being the dependent variable, data_imputed the explanatory variables
# In-sample RMSE
sqrt(mean((  ) ^ 2))#Question 2: calculate the in-sample RMSE

# Use the first 30 data points to fit the model and the rest to test
training <- data_imputed[, ] #Question 3a: define the training dataset
test <- data_imputed[, ] #Question 3b: define the test dataset
lm_fit2 <- lm(FwdReturn ~ ., data = training)
preds <- #Question 4a: make predictions for test, using lm_fit2
# Out-of-sample RMSE
#Question 4b: calculate the out of sample RMSE
...
lm_fit <- train(
  FwdReturn ~ .,
  data = training,
  method = "" #Question 5: specify method to get a linear model
)
lm_fit
# Change the method argument to "rf" to fit a random forest
rf_fit <- train(
  FwdReturn ~ .,
  data = training,
  method = #Question 6: specify method to grow a random forest
)
rf_fit
#Question 7a: make predictions with the training and test datasets.
#Question 7b: plot the FwdReturn of the training (resp. test) vs. the predictions
...
rf_fit1 <- train(
  ...
  na.action = na.pass, #!! Set na.action to pass
  method = "rf"
)
rf_fit1

# You can also pass multiple arguments to preProcess. E.g.:
# 1. remove columns with zero or near-zero variance ("nzv")
# 2. median impute missing values
# 3. center and 4. scale all variables
rf_fit2 <- train(
  FwdReturn ~ .,
  data = data,
  preProcess = ,#Question 8: use simultaneously "nzv", "medianImpute", "center" and "scale"
  na.action = na.pass, #!! Set na.action to pass
  method = "rf"
)

# Defining repeated cross-validation resampling scheme
# Define the resampling scheme using the trainControl function
my_cv_control <- trainControl(
  method = "repeatedcv",
...
rf_fit3 <- train(
  ...
  na.action = na.pass,
  method = "rf",
  trControl = my_cv_control # pass the trainControl object to trControl
)

# Time series validation
my_time_control <- trainControl (
  method = ,#Question 9a: use the timeslice method
  initialWindow = 60, # use the first 60 data points to train
  horizon = , #Question 9b: forecast one period ahead 
  fixedWindow = TRUE # use rolling window (the default)
)

# Simple tuning via tuneLength
set.seed(202202)
rf_fit4 <- train(
  FwdReturn ~ .,
  data = data,
...
# Lowest RMSE
#Question 10a: find the minimum RMSE of rf_fit4
#Question 10b: plot rf_fit4

# Custom tuning via tuneGrid
# Boosted trees (gbm package) have 4 tuning parameters
gbm_grid <- expand.grid(
  n.trees = seq(20, 100, by = 10),
...
gbm_fit <- train(
  ...
  method = "gbm",
  trControl = my_cv_control,
  tuneGrid = , #Question 12: use gbm_grid as the tuning grid
  verbose = FALSE # gbm output is verbose, may want to supress it
)
# Optimal paramters
gbm_fit$bestTune

# Lowest RMSE
min(gbm_fit$results$RMSE)
ggplot(gbm_fit)

# Variable importance, random forest model
# For rf, you need to explicitly set importance = TRUE in train()
set.seed(202202)
rf_fit <- train(
  FwdReturn ~ .,
  data = data,
...
# By default, scores are scaled between 0 and 100; to get raw scores, set scale to FALSE
rf_imp <- varImp(rf_fit, scale = FALSE)
ggplot(rf_imp) +
  ggtitle("Unscaled variable importance scores, random forest model ")
